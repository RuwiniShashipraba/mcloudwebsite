<?php
    ...
    if (isset($_SESSION["SESSION_EMAIL"])) {
        header("Location: doctorRegistrationForum.php");
    }
        ...
        try {
           ...
           $mail->SMTPDebug = 0;//SMTP::DEBUG_SERVER;
            ...
            $email_template = "
                                ...
                                <br/><br/>                     
                                <a href = 'https://m-cloud.herokuapp.com/user/doctor/verify_email.php?token=$token'> Click to Verify </a>";
    ...
    if (isset($_POST["submit"])) {
        ...
        if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM doctor WHERE Email='{$email}'")) > 0) {
            echo "<script>alert('{$email} - This email has already taken.');</script>";
        }else {
            if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM doctor WHERE NationalID='{$nid}'")) > 0){
                echo "<script>alert('{$nid} - This nationalID has already taken.');</script>";
            }else{
                if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM doctor WHERE DoctorID='{$did}'")) > 0){
                    echo "<script>alert('{$did} - This doctorID has already taken.');</script>";
                }else{
                    if ($password === $cpassword) {
                        $sql = "INSERT INTO doctor (Fullname, Username, Email, Password, Gender, TelNumber, NationalID, Address, DOB, Specilization, DoctorID, Token) VALUES ('{$name}', '{$username}','{$email}', '{$cpassword}', '{$gender}','{$phone}','{$nid}','{$address}','{$birth}','{$description}','{$did}','{$token}')";
...
<body>
    <div class="wrapper">
        ...
        <form action="" method="post" class="form">
            ...
            <div class="input-field">
                <label for="did" class="input-label">Doctor ID</label>
                <input type="text" name="did" id="did" class="input" placeholder="Enter your doctor id" required>
            </div>
            <div class="input-field">
            ...
            <div class="input-field">
                <label for="description" class="input-label">Specialization</label>
                <input type="text" name="description" id="description" class="input" placeholder="Enter specialization" required>
            </div>
            <div class="input-field">
            ...
            <p>You have already an account! <a href="doctorLoginForum.php">Login</a></p>
        </form>
    </div>
</body>
</html>
