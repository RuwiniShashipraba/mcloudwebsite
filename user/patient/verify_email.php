...
if(isset($_GET['token'])){
    ...
    if(mysqli_num_rows(mysqli_query($conn, "SELECT token FROM patient WHERE Token ='$token' LIMIT 1")) > 0)
    {
        $row = mysqli_fetch_assoc((mysqli_query($conn, "SELECT Token,Verify_status FROM patient WHERE Token ='$token' LIMIT 1")));
        if($row['Verify_status'] == '0'){
            $clicked_token = $row['Token'];
            $update_query = "UPDATE patient SET Verify_status = '1' WHERE Token = '$clicked_token' LIMIT 1";
